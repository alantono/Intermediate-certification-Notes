from data_base import  write_read_csv, read_data_1, write_data_1
import controller
...
def func_1():                                                   # Вывод всех Заметок в отдельном окне
    logger.log("Просмотрен список заметок")
    a = read_data_1()
...
def func_2(): # Добавление новой Заметки
    ...
    if controller.v1.get() == "" or controller.note_text.get(1.0, END) == "":
        controller.mistake_entry_1()     # вызывает сообщение о том что не заполнено поле Заглавие или текст
    else:                               
...
def func_3():     # Удаление записи тел книги по фамилии или имени
    ...
    if del_rec not in notes_list:
        ...
        write_data_1(notes_list)                                    # после удаления Заметки записали обновленный список в файл Notebook.txt
...
def func_4():     # Редактирование записи в тел книге (кнопка Редактировать)
    ...
    notes_list = read_data_1()                                      # считываем последнюю редакцию списка заметок
    if ed_rec not in notes_list or ed_rec == "":
        controller.mistake_entry()                                  # вызывает сообщение о том что Заметки нет в списке
    else:
...
def func_4_1(): # Редактирование записи в тел книге (кнопка Сохранить изменения).
    ...
    notes_list[index + 1] = str((' '.join((controller.note_text.get(1.0, END)).split(" "))).strip()) # записываем в поле Текст отредактированный текст заметки
...
def func_5():  # Поиск Заметки по дате (кнопка показать)
    ...
    list = []
    notes_list = read_data_1() # считываем из файла Nb.txt последнюю редакцию списка заметок
    if controller.v4.get() not in notes_list or controller.v4.get() == "":
